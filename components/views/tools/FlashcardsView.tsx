...
import { collection, query, where, onSnapshot, addDoc, deleteDoc, doc, Timestamp, orderBy, writeBatch, getDocs, increment, updateDoc, limit } from '@firebase/firestore';
...
type ViewType = 'subjects' | 'decks' | 'mode-selection' | 'manage' | 'learn' | 'cram' | 'mc' | 'summary' | 'all-learned';
...
const FlashcardsView: React.FC<FlashcardsViewProps> = (props) => {
  ...
  const Views: { [key: string]: React.ReactNode } = {
    ...
    decks: selectedSubject && <DeckSelectionView {...props} selectedSubject={selectedSubject} setSelectedDeck={setSelectedDeck} setView={setView} onBack={goBack} />,
    'mode-selection': selectedDeck && <ModeSelectionView deck={selectedDeck} setView={setView} onBack={goBack} {...props} />,
    manage: selectedDeck && <CardManagerView {...props} deck={selectedDeck} onBack={goBack} />,
    learn: selectedDeck && <LearnSessionView {...props} deck={selectedDeck} setView={setView} onSessionComplete={handleSessionComplete}/>,
    cram: selectedDeck && <CramSessionView {...props} deck={selectedDeck} setView={setView} onSessionComplete={handleSessionComplete}/>,
    mc: selectedDeck && <MultipleChoiceSessionView {...props} deck={selectedDeck} setView={setView} onSessionComplete={handleSessionComplete}/>,
    summary: selectedDeck && lastSessionStats && lastSessionMode && <SessionSummaryView {...props} deck={selectedDeck} stats={lastSessionStats} setView={setView} onBack={goBack} mode={lastSessionMode}/>,
    'all-learned': selectedDeck && <AllCardsLearnedView {...props} deck={selectedDeck} setView={setView} />
  }
...
const DeckSelectionView: React.FC<any> = ({ onBack, selectedSubject, setSelectedDeck, setView, ...props }) => {
    const { userId, t, tSubject, getThemeClasses, showAppModal, user } = props;
    const [decks, setDecks] = useState<FlashcardDeck[]>([]);
    const [newDeckName, setNewDeckName] = useState('');

    useEffect(() => {
        if (!selectedSubject || user.uid === 'guest-user') { setDecks([]); return; }
        ...
        const unsubscribe = onSnapshot(q, snapshot => {
            ...
            showAppModal({text: t('error_failed_to_load_decks')});
        });
        return () => unsubscribe();
    ...
    const handleCreateDeck = async (e: React.FormEvent) => {
        ...
        if (!newDeckName.trim()) { showAppModal({ text: t('error_empty_deck_name') }); return; }
        await addDoc(collection(db, `artifacts/${appId}/users/${userId}/flashcardDecks`), { name: newDeckName, subject: selectedSubject, ownerId: userId, createdAt: Timestamp.now(), cardCount: 0 });
        showAppModal({ text: t('deck_added_success') });
        setNewDeckName('');
    };
    
    const handleDeleteDeck = async (deck: FlashcardDeck) => {
        showAppModal({ text: t('confirm_delete_deck', { name: deck.name }),
            confirmAction: async () => {
    ...
    return (
        <div className={`p-4 rounded-lg shadow-inner ${getThemeClasses('bg-light')} space-y-4`}>
            ...
            <form onSubmit={handleCreateDeck} className="bg-white p-3 rounded-lg shadow-sm flex gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {decks.length === 0 ? <p className="text-center italic text-gray-500 py-4 md:col-span-2">{t('no_decks_found')}</p> :
                 decks.map(deck => (
                    <div key={deck.id} onClick={() => { setSelectedDeck(deck); setView('mode-selection'); }} className="bg-white p-4 rounded-lg shadow-md flex flex-col justify-between cursor-pointer hover:shadow-lg hover:-translate-y-1 transition-all">
                        <div>
                            <p className="font-bold text-lg">{deck.name}</p>
                            <p className={`text-sm font-semibold ${getThemeClasses('text')}`}>{t('cards_in_deck', { count: deck.cardCount || 0 })}</p>
                        </div>
                        <div className="flex justify-between items-center mt-4 gap-2">
                             <button onClick={(e) => { e.stopPropagation(); handleDeleteDeck(deck); }} className="p-2 text-red-500 bg-red-100 hover:bg-red-200 rounded-md transition-colors active:scale-90"><Trash2 className="w-4 h-4"/></button>
                             <button onClick={(e) => { e.stopPropagation(); setSelectedDeck(deck); setView('manage'); }} className={`flex items-center gap-2 font-semibold text-sm py-2 px-3 rounded-lg transition-colors active:scale-95 bg-gray-200 hover:bg-gray-300`}><Settings size={16}/> {t('manage_deck')}</button>
                        </div>
                    </div>
                ))}
            </div>
...
const ModeSelectionView = ({ onBack, deck, setView, ...props }: any) => {
    ...
    const handleResetProgress = () => {
        showAppModal({
            ...
            confirmAction: async () => {
                const batch = writeBatch(db);
                const cardsRef = collection(db, `artifacts/${appId}/users/${userId}/flashcardDecks/${deck.id}/cards`);
    ...
    return (
      <div className={`p-4 rounded-lg shadow-inner ${getThemeClasses('bg-light')} space-y-4`}>
          <div className="flex justify-between items-center">
            <button onClick={onBack} className="font-semibold flex items-center gap-1 hover:underline"><ArrowLeft size={16}/> {t('back_to_decks')}</button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {studyModes.map(mode => (
                 <div key={mode.id} onClick={() => deck.cardCount > 0 ? setView(mode.id) : showAppModal({text: t('no_flashcards_found')})} className="bg-white rounded-xl shadow-lg p-6 flex flex-col items-center text-center cursor-pointer hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
                  ...
                  <button disabled={deck.cardCount === 0} className={`mt-4 w-full font-bold py-2 px-4 rounded-lg text-white ${getThemeClasses('bg')} ${getThemeClasses('hover-bg')} disabled:opacity-50 disabled:cursor-not-allowed`}>{t('start_session')}</button>
              </div>
              ))}
          </div>
      </div>
...
const CardManagerView = ({ onBack, deck, ...props }: any) => {
    ...
    const [cardRows, setCardRows] = useState(Array.from({ length: 5 }, () => ({ question: '', answer: '' })));
     ...
     return (
        <div className={`p-4 rounded-lg shadow-inner ${getThemeClasses('bg-light')} space-y-4`}>
            <button onClick={onBack} className="font-semibold flex items-center gap-1 hover:underline"><ArrowLeft size={16}/> {t('back_to_decks')}</button>
            <h3 className="font-bold text-lg text-center">{t('add_flashcard')} - {deck.name}</h3>
...
const CramSessionView = ({ deck, onSessionComplete, ...props }: any) => {
    ...
    useEffect(() => {
        ...
        const q = query(collection(db, `artifacts/${appId}/users/${userId}/flashcardDecks/${deck.id}/cards`), orderBy('createdAt'));
        const unsubscribe = onSnapshot(q, snapshot => {
        ...
        return () => unsubscribe();
    }, [userId, deck.id, user.uid, showAppModal, t]);
    ...
    return <StudyCardLayout card={card} isFlipped={isFlipped} setIsFlipped={setIsFlipped} progressPercentage={progressPercentage} deck={deck} setView={setView} t={t} getThemeClasses={getThemeClasses} cardsRemainingText={`${currentIndex + 1} / ${cards.length}`}>
...
const LearnSessionView = ({ deck, onSessionComplete, ...props}: any) => {
    const { userId, user, t, getThemeClasses, showAppModal, setView } = props;
    ...
    useEffect(() => {
        if (user.uid === 'guest-user') { setIsLoading(false); return; }
        const q = query(
            collection(db, `artifacts/${appId}/users/${userId}/flashcardDecks/${deck.id}/cards`),
    ...
    const updateCardSrs = async (card: Flashcard, knewIt: boolean) => {
        ...
        const cardRef = doc(db, `artifacts/${appId}/users/${userId}/flashcardDecks/${deck.id}/cards`, card.id);
    ...
    useEffect(() => {
        if (!isLoading && sessionQueue.length === 0 && dueCards.length > 0) {
            ...
            const q = query(collection(db, `artifacts/${appId}/users/${userId}/flashcardDecks/${deck.id}/cards`), where('dueDate', '<=', Timestamp.now()), limit(1));
    ...
    }, [isLoading, sessionQueue.length, dueCards.length, onSessionComplete, stats, setView, userId, deck.id]);

    if (isLoading) return <div className="text-center p-8 flex justify-center items-center gap-2"><Loader2 className="animate-spin" /> Loading cards...</div>;
    if (dueCards.length === 0) return <AllCardsLearnedView {...props} deck={deck} setView={setView} />

    return <StudyCardLayout card={card} isFlipped={isFlipped} setIsFlipped={setIsFlipped} progressPercentage={progressPercentage} deck={deck} setView={setView} t={t} getThemeClasses={getThemeClasses} cardsRemainingText={t('cards_to_go', {count: sessionQueue.length})} >
...
const MultipleChoiceSessionView = ({ deck, onSessionComplete, ...props }: any) => {
    const { userId, user, t, getThemeClasses, showAppModal, setView } = props;
    ...
    useEffect(() => {
        if (user.uid === 'guest-user') { setIsLoading(false); return; }
        const q = query(collection(db, `artifacts/${appId}/users/${userId}/flashcardDecks/${deck.id}/cards`));
        const unsubscribe = onSnapshot(q, snapshot => {
            const fetchedCards = snapshot.docs.map(d => ({id: d.id, ...d.data()} as Flashcard));
            if (fetchedCards.length < 2) {
                showAppModal({ text: "You need at least 2 cards in a deck for Multiple Choice mode." });
                setView('mode-selection');
                return;
            }
        ...
        return () => unsubscribe();
    }, [userId, deck.id, user.uid, showAppModal, t, setView]);
    ...
    return (
        <div className={`p-4 rounded-lg shadow-inner ${getThemeClasses('bg-light')} space-y-4 flex flex-col`}>
             <button onClick={() => setView('mode-selection')} className="font-semibold flex items-center gap-1 hover:underline self-start mb-2"><ArrowLeft size={16}/> {t('back_to_decks')}</button>
             <h3 className="font-bold text-center text-xl">{deck.name}</h3>
...
const StudyCardLayout = ({ children, card, isFlipped, setIsFlipped, progressPercentage, deck, setView, t, getThemeClasses, cardsRemainingText }: any) => {
...
const NoCardsFoundView = ({setView, t, getThemeClasses }: any) => (
    <div className={`p-6 rounded-lg shadow-inner ${getThemeClasses('bg-light')} space-y-4 text-center`}>
        ...
        <div className="flex justify-center gap-4 mt-4">
             <button onClick={() => setView('decks')} className="font-semibold flex items-center gap-1 hover:underline"><ArrowLeft size={16}/> {t('back_to_decks')}</button>
             <button onClick={() => setView('manage')} className={`flex items-center gap-2 font-semibold text-sm py-2 px-3 rounded-lg transition-colors active:scale-95 text-white ${getThemeClasses('bg')} ${getThemeClasses('hover-bg')}`}><Settings size={16}/> {t('manage_deck')}</button>
        </div>
    </div>
);

const AllCardsLearnedView = ({ deck, setView, ...props }: any) => {
