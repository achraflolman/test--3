...
import { User, Palette, Info, HelpCircle, Shield, GripVertical, Plus } from 'lucide-react';
        ...
        <SettingSection title={t('profile_section')} icon={<User className={getThemeClasses('text')} />}>
            ...
            <div className="pt-4 border-t">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {allDisplaySubjects.map(s => <button key={s} onClick={() => handleSubjectToggle(s)} className={`p-2 rounded-lg text-sm transition-all duration-200 active:scale-95 ${selectedSubjects.includes(s) ? getThemeClasses('bg') + ' text-white shadow' : 'bg-gray-100 hover:bg-gray-200'}`}>{tSubject(s)}</button>)}
              </div>
            </div>
